...
def algo(data):
    ...
    for b in b_list:
        ...
        Z = optimalZonotopeL1Norm(A,b,G,C,d_new) #calulate optimal center and scaling limits
...
def optimalZonotopeL1Norm(A,b,G,F,bi_list):
    AG = np.abs(A@G)
    ...
    model.Params.OutputFlag = 0
    t = model.addMVar(len(delta_p),lb=-gp.GRB.INFINITY)
    c = model.addMVar(shape = np.shape(A)[1],lb=-gp.GRB.INFINITY)
    beta_bar = model.addMVar(shape = np.shape(G)[1],lb=0.0)
    aux_1 = model.addMVar(shape = np.shape(F)[0],lb=-gp.GRB.INFINITY)
    aux_2 = model.addMVar(shape = np.shape(W_aux)[0],lb=-gp.GRB.INFINITY)
    model.setObjective(sum(t),gp.GRB.MINIMIZE)
    model.addConstr(aux_1 == F@c)
    model.addConstr(aux_2 == W_aux@beta_bar)
    for i in range(len(delta_p)):
        model.addConstr(-t[i] <= delta_p[i]-(aux_1[i] + aux_2[i])        )
        model.addConstr(         delta_p[i]-(aux_1[i] + aux_2[i]) <= t[i])
