...
def algo(data):
    ...
    for b in b_list:
        ...
        Z = optimalZonotopeL2Norm(A,b,G,C,d_new) #calulate optimal center and scaling limits
...
def optimalZonotopeL2Norm(A,b,G,F,bi_list):
    AG = np.abs(A@G)
    W = np.abs(F[0:int(np.shape(F)[0]/2)]@G)
    delta_p = np.array(bi_list)
    W_aux = np.row_stack((W,W))
    
    c = cvx.Variable([np.shape(A)[1],1])
    beta_bar = cvx.Variable([np.shape(G)[1],1])
    obj = cvx.Minimize(cvx.norm2(delta_p.reshape(len(delta_p),1) - (F@c + W_aux@beta_bar)))
    
    constraints = [beta_bar >= 0]
    constraints += [AG@beta_bar + A@c <= b.reshape(len(b),1)]
    prob = cvx.Problem(obj, constraints)
    prob.solve()
    
    c = c.value.flatten()
    beta_bar = beta_bar.value.flatten()
