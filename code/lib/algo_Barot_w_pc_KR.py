...
def algo(data):
    """
    Alogrithm: Barot outer aproximation without preconditioning
    ...
    # preconditioning:
    for h in range(H):
        model = gp.Model('Barot preconditioning')
        model.Params.OutputFlag = 0
        x = model.addMVar(shape=T, lb=-gp.GRB.INFINITY, vtype=gp.GRB.CONTINUOUS, name="x")
        model.addConstr( A@x <= b[:,h] )
        for r in range(A.shape[0]):  # for each row in A
            model.setObjective(A[r,:]@x, gp.GRB.MAXIMIZE)
            model.optimize()
            b_r_opt = model.objVal
            # print(f"{b[r,h] - b_r_opt:.2f}", end=", ")
            b[r,h] = b_r_opt
    # sum up right hand side vectors:
    b_approx = np.sum(b, axis=1)
    ...
    for obj in data['objectives']:
        if obj == 'cost':
            # optimization:
            t0 = time.time()
            model = gp.Model('Barot without preconditioning, cost')
            model.Params.OutputFlag = 0
            x = model.addMVar(shape=T, lb=-gp.GRB.INFINITY, vtype=gp.GRB.CONTINUOUS, name="x")  
