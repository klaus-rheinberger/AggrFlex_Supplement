# -*- coding: utf-8 -*-
"""
Created on Thu Jan 27 12:14:55 2022
...
def algo(data):
    """
    Alogrithm: Zhen Inner
    
    References:
    - Zhen, J.; den Hertog, D. Computing the Maximum Volume Inscribed Ellipsoid of a Polytopic Projection. INFORMS Journal on 395 Computing 2018, 30, 31–42. doi:10.1287/ijoc.2017.0763. 
    """
    ...
    E = cvx.Variable([T,T],symmetric=True)
    V = cvx.Variable([n-T,T])
    x = cvx.Variable([T,1])
    u = cvx.Variable([n-T,1])
    obj = cvx.Maximize(cvx.log_det(E))
    
    constraints = [E >> 0]
    constraints += [cvx.norm(cvx.vstack([E,V]).T@A_proj[i,:]) + A_proj[i,:].T@cvx.vstack([x,u]) <= b_proj[i] for i in range(m)]
