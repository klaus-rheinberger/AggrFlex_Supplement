# -*- coding: utf-8 -*-
"""
Created on Thu Feb 17 13:30:35 2022
...
def algo(data):
    ...
    for b in b_list:
        ...
        Z = optimalZonotopeRel(A,b,G,C,d_new) #calulate optimal center and scaling limits
...
def optimalZonotopeRel(A,b,G,F,bi_list):
    AG = np.abs(A@G)
    #W = np.abs(F[0:int(np.shape(F)[0]/2)]@G)
    delta_p = np.array(bi_list)
    #W_aux = np.row_stack((W,W))
    
    delta_p_aux = 1/delta_p
    matrix_aux = np.abs(F@G)
    w = 2/(np.shape(F)[0]/2)*(delta_p_aux@matrix_aux)
    ...
    model.setObjective(w@beta_bar,gp.GRB.MAXIMIZE)
