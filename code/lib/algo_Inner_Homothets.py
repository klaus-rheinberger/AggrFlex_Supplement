# -*- coding: utf-8 -*-
"""
Created on Thu Jan 27 11:04:09 2022
...
def algo(data):
    ...
    for b in b_list:
        beta,t = fitHomothet(A,b,b_mean,True,T) # caluclate optimal scaling factor and offset
    ...
    for obj in data['objectives']:
        if obj == 'cost':
            t0 = time.time()
            model = gp.Model('Homothets Inner, cost')
            ...
            t0 = time.time()
            model = gp.Model('Homothets Inner, peak')
            model.Params.OutputFlag = 0
            x = model.addMVar(shape=T, lb=-gp.GRB.INFINITY, vtype=gp.GRB.CONTINUOUS, name="x")
